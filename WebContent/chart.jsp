...
<div id="vie">View Disease Based Gender Analyze</div>
<div id="h2">
<select name="disease" id="reg">		
<% 
...
try{
	...
	String sql1="select disease from upload group by disease ";
	rs2=st2.executeQuery(sql1);
	while(rs2.next()){
		%>
		<option value="<%=rs2.getString("disease")%>"><%=rs2.getString("disease")%>
		<% 
	}
	%>
	</option>
...
Statement st=null;
try{
	String g=request.getParameter("disease");
	con=treatement.getconnection();
	st=con.createStatement();
	String sql="select gender,count(gender) as numbers ,age,gender,pressure,cholestral,sugar,electrocardiograph,heartrate,angina,strest,symtoms,disease from upload where disease='"+g+"' group by gender";
	rs=st.executeQuery(sql);
		while(rs.next()){
			g=rs.getString("disease");
			aa=rs.getString("gender");
	...
	window.onload = function () {
		var chart = new CanvasJS.Chart("chartContainer", {
			theme: "theme2",//theme1
			title:{
				text: "Disease Based GenderAnalzye"              
			},
